...
export async function pauseProjectSandbox(projectId: string) {
    ...
    if (!sandboxRecord) return { status: SandboxStatus.PAUSED } as const;

    try {
        await SandboxApi.betaPause(sandboxRecord.sandboxId);
        await prisma.projectSandbox.update({
            where: { projectId },
            data: { status: SandboxStatus.PAUSED, lastActiveAt: new Date() },
        });
    } catch (error) {
        if (error instanceof NotFoundError) {
            return { status: SandboxStatus.PAUSED, recreated: true } as const;
        }

        throw error;
    }

    return { status: SandboxStatus.PAUSED } as const;
}
